...
let countLikes = state.countLikes.value.length
	...
	flex  justify-center w-[100%] min-h-[50px] fixed z-20 rounded border-b-[2px] rounded border-cyan-300 outline outline-offset-2 outline-pink-500 z-50 items-center`}>
		...
		`}>
				...
				<div className={`flex max-w-[300px] justify-around  h-[90%] align-middle 
				...
				id={`text-shadow-p`}
				>
				<NavLink to='project' className={`cusor-pointer z-10 items-center mr-2 `}>Главная</NavLink>

				<NavLink to='products' className={`cusor-pointer z-10 mr-2`}>Товары</NavLink>

				<NavLink to='Delevery' className={`cusor-pointer z-10 `}>Доставка</NavLink>
		...
		">
			<NavLink to="Buscket">
				<div className={`
				items-gradient icon
				flex cusor-pointer cusor-pointer justify-around relative rounded items-center pl-[2%] pr-[2%]
				${state.switchTeamSlice.nightTeam? 'bg-white' : 'bg-[#260b2d]'} 
				sm:mr-5
				`}
				id={`box-shadow`}
				>
